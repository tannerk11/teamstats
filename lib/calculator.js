...
export function calculateCustomStats(teamsData, splitType = 'conference') {
  return teamsData
    ...
    .map(team => {
      ...
      const fgmpgopp = parseFloat(stats.fgmpgopp) || 0;
      const fgm3pgopp = parseFloat(stats.fgm3pgopp) || 0;
      const fga3pgopp = parseFloat(stats.fga3pgopp) || 0;
      ...
      const ftmpgopp = parseFloat(stats.ftmpgopp) || 0;
      ...
      // Opponent Effective FG % = (Opp FGM + 0.5 * Opp 3PM) / Opp FGA
      const efgPctOpp = fgapgopp > 0 ? ((fgmpgopp + (0.5 * fgm3pgopp)) / fgapgopp) * 100 : 0;
      
      // Opponent FG% = (Opp FGM / Opp FGA) * 100
      const fgPctOpp = fgapgopp > 0 ? (fgmpgopp / fgapgopp) * 100 : 0;
      
      // Opponent 3PT% = (Opp 3PM / Opp 3PA) * 100
      const fg3PctOpp = fga3pgopp > 0 ? (fgm3pgopp / fga3pgopp) * 100 : 0;
      
      // Opponent FT% = (Opp FTM / Opp FTA) * 100
      const ftPctOpp = ftapgopp > 0 ? (ftmpgopp / ftapgopp) * 100 : 0;
      
      // Turnover % = TO / Possessions
      const toPct = possessions > 0 ? (topg / possessions) * 100 : 0;
      
      // Opponent Turnover % = Opp TO / Opp Possessions
      const toPctOpp = oppPossessions > 0 ? (topgopp / oppPossessions) * 100 : 0;
      ...
      // Opponent Offensive Rebound % = Opp OREB / (Opp OREB + DREB)
      const orPctOpp = (orebpgopp + drebpg) > 0 ? (orebpgopp / (orebpgopp + drebpg)) * 100 : 0;
      
      // Opponent Defensive Rebound % = Opp DREB / (Opp DREB + OREB)
      const drPctOpp = (drebpgopp + orebpg) > 0 ? (drebpgopp / (drebpgopp + orebpg)) * 100 : 0;
      
      // Free Throw Rate = FTA / FGA (as percentage)
      const ftRate = fgapg > 0 ? (ftapg / fgapg) * 100 : 0;
      
      // Opponent Free Throw Rate = Opp FTA / Opp FGA (as percentage)
      const ftRateOpp = fgapgopp > 0 ? (ftapgopp / fgapgopp) * 100 : 0;
      
      // 3-Point Attempt Rate = (3PA / FGA) * 100
      const threePtRate = fgapg > 0 ? (fga3pg / fgapg) * 100 : 0;
      
      // Opponent 3-Point Attempt Rate = (Opp 3PA / Opp FGA) * 100
      const threePtRateOpp = fgapgopp > 0 ? (fga3pgopp / fgapgopp) * 100 : 0;
      
      // Shot Volume = (FGA + 0.475 * FTA) / Possessions
      const shotVolume = possessions > 0 ? (fgapg + (0.475 * ftapg)) / possessions : 0;
      
      // Opponent Shot Volume = (Opp FGA + 0.475 * Opp FTA) / Opp Possessions
      const shotVolumeOpp = oppPossessions > 0 ? (fgapgopp + (0.475 * ftapgopp)) / oppPossessions : 0;
      ...
      return {
        ...
        fgptOpp: parseFloat(fgPctOpp.toFixed(1)),
        fgpt3Opp: parseFloat(fg3PctOpp.toFixed(1)),
        ftptOpp: parseFloat(ftPctOpp.toFixed(1)),
