...
import { aggregateStats } from './eventFilter.js';
...
function calculateWinLoss(events = [], splitType = 'conference') {
  ...
  events.forEach(event => {
    // Skip exhibition and pre-season games (same filter as eventFilter.js)
    ...
    const isDivision = event.event?.division === true;
    const isNational = event.event?.national === true;
    
    // Skip if not matching the requested split type
    if (splitType === 'conference' && !isConference) {
      return;
    }
    if (splitType === 'division' && !isDivision) {
      return;
    }
    if (splitType === 'national' && !isNational) {
      return;
    }
    // Note: 'overall' split type doesn't filter - includes all games (except exhibition)
...
export function calculateCustomStats(teamsData, splitType = 'conference') {
  // Import filterEvents and aggregateStats from eventFilter
  return teamsData
    ...
    .map(team => {
      ...
      // Filter events based on split type and exclude exhibition games
      const filteredEvents = events.filter(event => {
        ...
        // 'overall' includes all games (except exhibition)
